(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('tennisBuddiesApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-tennisBuddiesApp-error');
                var entityKey = httpResponse.headers('X-tennisBuddiesApp-params');
                if (errorHeader) {
